...
**클코 담당:**
1. 방금 고친 `/admin/analytics`·재입고 알림 탭이 배포 환경에서 실제로 정상 표시되는지 확인(리포트해주신 분이니 재확인도 부탁).
2. 주문 상태 변경 UI 보강 — `admin_update_order_status` RPC가 이제 취소류 주문의 역방향 전이를 거부하고 `409 + message`를 반환합니다. 관리자 주문 화면에서 이 메시지를 그대로 노출하고, 가능하면 취소류 상태인 주문은 드롭다운에서 다른 상태 선택 자체를 비활성화해주세요(아래 "🔍 전체 시스템 점검" 섹션의 "클코에게 지시" 2번 항목).

**안티 담당:**
1. 학습 보상 설정값(`site_settings`, category=`reward`: `study_points_per_review`/`study_daily_point_cap`/`study_reward_mode`) 관리자 UI 노출 — 기존 사이트 설정 관리 화면이 있으면 거기 3행만 추가, 없으면 간단한 폼 하나로 충분합니다.
...
**클코·안티 공통 (급하지 않음, 시간 날 때):** `AdminDashboard.tsx`의 메인 `fetchData` 콘솔 무음 실패 패턴 정리, `product_options` RLS 옛날 이메일 allowlist 정리.
