...
**[2026-08-01, claude-code] 일복님용 실행 체크리스트 페이지 신설**: https://app.notion.com/p/3af1fb8202158135b2dcc0f865eca2fb — 위 기술 DB를 일복님이 순서대로 클릭하며 처리하실 수 있게 우선순위별 체크박스 목록으로 재정리(위 DB의 하위 페이지). 일복님이 전화번호/이메일 알려주시면 코드는 제가 바로 반영, PortOne/Resend/사업자정보 확인/Supabase 대시보드 설정은 일복님 직접 처리 항목으로 구분해뒀음. 처리 완료 항목은 이 파일과 기술 DB에도 반영 필요.

**[2026-08-01] 노션 사용 통일 양식 — "업데이트 로그" DB 신설**: https://app.notion.com/p/9799fb6800334f7a81acb4a31c9fafed (위 대시보드 페이지 하위) — 일복님이 "코웍/단코/클코가 노션 쓸 때 통일된 폼으로 써달라"고 요청. **앞으로 노션에 뭔가 남길 땐 자유형식 페이지를 새로 만들지 말고 이 DB에 행을 추가**해주세요.
