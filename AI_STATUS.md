...
**Cowork-Claude가 설계 리드 겸 지휘자**입니다(2026-08-02, 일복님이 4자 협업 체제로 확장하며 명시적으로 지정). 새 테이블/컬럼/RLS/API 계약처럼 여러 화면에 영향 주는 변경은 이 파일에 제안 후 Cowork 확인을 거칩니다. 화면 안에서 끝나는 UI/UX 작업은 각 세션이 자율 진행 후 이 파일에 요약만 남기면 됩니다. 의견 충돌 시, 또는 클코·안티가 같은 작업을 동시에 잡으려 할 때 최종 조정은 Cowork-Claude가 하되, 일복님이 언제든 뒤집을 수 있습니다.

**[2026-08-15 갱신] 역할 무게중심 이동 — 직접 구현 → 시니어 검토/감독/보고.** 일복님 지시: "실무자에게 업무를 양보하고 전체적으로 일을 잘하고 있는지 감시하고 보고해라." 이제부터:
- **이 저장소(쇼핑몰) 안에서 코웍이 유일한 백엔드 담당인 영역**(DB/RLS/결제/재고 등)은 지금까지처럼 직접 구현하되, 우선순위는 "직접 고치기"보다 "실무자가 처리할 수 있으면 위임하고 나는 검토"에 둔다.
- **다른 세션의 저장소(예: Solo ERP)에서 발견한 이슈는 직접 고치지 않는다.** 그 저장소의 `AI_STATUS.md`에 리뷰 코멘트로 남기고, 실제 수정은 그 실무자(해당 세션)에게 맡긴다.
- 코웍은 정기적으로 이 파일 + 관련 저장소들을 훑어 **전체가 잘 굴러가는지 감독**하고, 발견한 이슈/리스크/막힌 지점을 일복님께 요약 보고한다. 백로그 정리·작업 배정(지휘자 역할)은 계속 수행.
