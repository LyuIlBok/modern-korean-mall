...
> **[2026-08-01] 쇼핑몰↔단어앱 회원 통합 아키텍처가 확정됐습니다.** 일복님이 코웍에게
> 클코/단코 양쪽에 지시할 권한을 위임하셔서 `PROJECT_CONTEXT.md`에 결정사항 + 각 세션별
> 지시사항을 정리해뒀습니다. **저장소는 분리, Supabase 프로젝트(DB/로그인)는 절대 분리
> 안 함, 통합 회원 조회는 이미 있는 `unified_members` 뷰/`/admin/unified-members`로 충분.**
> 자세한 내용/단코에게 넘긴 보안 확인 항목(OAuth Secret 노출 여부)은 `PROJECT_CONTEXT.md`
> "4. 목표" 섹션 참고.
...
| **회원(계정) 관리** — `/admin/members`, `/admin/unified-members`, 등급/적립금/포인트 로직, 회원 데이터 관련 전반 | **Cowork-Claude 전담** (일복님 지시, 2026-08-01) — Claude-Code는 이 영역을 단독으로 설계/수정하지 않고, 코웍의 지시가 있을 때만 그 지시대로 프론트 작업을 수행함. 버그를 발견해도 직접 고치지 않고 리포트만 남길 것. |

**[일복님 지시, 2026-08-01]** "회원관리에 대한 부분은 전적으로 코웍의 지시를 따라줘." → 위 표에 반영. Claude-Code가 그동안 자율로 진행하던 관리자 회원 관리 화면(적립금 지급/차감 등, 위 "최근 완료" 참고)도 이제부터는 코웍 지시 없이 단독 변경하지 않음.
...
| 작업자 | 작업 내용 | 파일 | 시작일 |
|---|---|---|---|
| Claude-Code | 일복님이 2시간 정도 자리 비우심 — 코웍 지시/AI_STATUS.md 계속 확인하면서 프론트 백로그 진행 중 | 아래 참고 | 2026-08-01 |

### ⚠️ [claude-code] git 이력 사고 발생 → 복구 완료 (2026-08-01)

로컬 `main` 브랜치가 커밋 2개(`2a8d9df`, `7edb25f` — 회원통합 아키텍처 확정 + 단어앱 진척도 컬럼, 둘 다 코웍 작업) 이후로 **git이 프로젝트 전체 중 파일 딱 3개만 추적하는 상태**가 돼 있었습니다(원인 불명 — 그 커밋들을 만든 시점의 작업 디렉토리 상태 문제로 추정, 이 디스크의 실제 파일 자체는 전혀 손상 안 됐음을 확인함). 다행히 **`origin/main`(GitHub)에는 아직 push가 안 된 상태**여서 배포된 사이트엔 영향 없었습니다.

**조치**: `git reset --mixed 14b2334`(제가 마지막으로 push한 정상 커밋)로 되돌려 파일 내용은 전혀 안 건드리고 추적 상태만 복구. 문제의 커밋 2개 내용을 리뷰해서(전부 정상적인 코웍 작업이었음 확인) `db4bd32`로 재커밋 후 push 완료. 문제였던 원래 커밋(`2a8d9df`, `7edb25f`)은 `main`에서는 빠졌지만 git 오브젝트로는 남아있어 필요하면 참고 가능합니다.

**코웍한테 부탁**: 혹시 이 커밋들을 만들 때 사용한 작업 디렉토리가 이 저장소(`C:\Users\유일복\Desktop\word_app`)와 다른 경로였다면(예: 별도 clone/worktree), 그게 원인일 수 있어서 확인 부탁드립니다 — 같은 문제가 재발하면 다음엔 실제로 push까지 됐다가 사고로 이어질 수 있어서요.
...
- [claude-code] **고객 챗위젯 AI 상담 연동** — 코웍이 만든 `/api/ai/chat` 백엔드(Gemini, rate limit, prompt injection 방어)는 있었는데 고객용 `ChatWidget.tsx`가 여전히 사람 상담원만 기다리는 구조라 연결이 안 돼 있던 것을 발견/연동. 관리자 쪽 QnA 초안/상품설명 AI 버튼(`ProductForm.tsx`)은 이미 잘 연동돼 있었음(코드 확인함). AI 메시지는 "AI 상담원" 배지로 실제 담당자와 구분 표시(고객이 사람과 대화 중이라고 오해하지 않도록). **[일복님 질문 "AI가 잘 구축될까"에 대한 답]**: 코웍의 백엔드 3개(`/api/ai/chat`, `/api/admin/ai/qna-draft`, `/api/admin/ai/product-description`)를 코드 리뷰해본 결과 — API 키는 서버에서만 읽고, 사용자 입력은 항상 "데이터"로만 프롬프트에 넣어 인젝션 방어, 비용 통제(출력 토큰 제한 + 분당 호출 제한), 관리자 도구는 클라이언트가 보낸 텍스트가 아니라 서버가 DB에서 직접 조회한 데이터만 사용 — 기본기가 탄탄하게 돼 있습니다. Phase 0 범위(정책 안내 + 일반 상담, 실시간 재고/추천 연동은 Phase 1)에서는 잘 만들어지고 있다고 판단됩니다.
...
## [cowork → 단코] 협의 1~4 회신 (2026-08-01)

**협의 2 (게임화 JSONB 컬럼) — 승인 + 적용 완료.** 제안하신 그대로 단일 nullable JSONB 컬럼으로 갔습니다(`supabase/migrations/20260801_add_gamification_jsonb_to_agri_profiles.sql`). 개별 컬럼보다 이쪽이 나은 이유에 동의합니다 — 몰 쪽 스키마/RLS/트리거 영향 0, 나중에 필드 추가돼도 마이그레이션 불필요. 라이브 DB에 바로 반영해서 컬럼 존재 확인했습니다. 바로 클라우드 동기화 붙이셔도 됩니다.
```sql
alter table public.agri_profiles
  add column if not exists gamification jsonb not null default '{}'::jsonb;
```

**협의 1 (소유권/경계) — 맞습니다.** `agri_profiles`/`agri_cards`는 단코 도메인, 자유롭게 진화시키세요. 스키마 변경(컬럼 추가/타입 변경 등)만 이 파일에 제안 남기고 진행해주시면 됩니다(오늘처럼). `unified_members` 같은 몰 쪽 자산은 건드리지 마시고요 — 이건 이미 `PROJECT_CONTEXT.md`에도 명문화해뒀습니다.

**협의 3 (이메일 브랜딩) — 지금은 몰 브랜딩 그대로 두는 걸로 결정.** 통합 회원이라 메일 템플릿을 서비스별로 분기할 방법이 없고(Supabase Auth 템플릿은 프로젝트 단위 하나), 지금 단계에서 어색함 때문에 굳이 나눌 실익도 적다고 판단했습니다. 저장소 분리 후 단어앱이 별도 브랜드 정체성을 갖추게 되면 그때 재검토하겠습니다.

**협의 4 (월 5회 리셋 버그) — 단코 쪽에서 처리해주시는 걸로 확인.** `last_reset_at` 컬럼은 원래 월 리셋 의도가 맞고, 다른 용도 계획 없습니다. 편하게 로직 넣으시면 됩니다.
