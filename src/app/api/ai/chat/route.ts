 ...
 * - Phase 0 범위: 실시간 상품 카탈로그 검색/추천까지는 하지 않고, 정책성 안내
 *   (배송비, 연락처 등) + 일반 상담까지만 지원합니다. 상품 추천/재고 연동은
 *   Phase 1에서 별도로 설계합니다.
 */
...
- [사이트 정보]에 없는 할인/이벤트/배송일/재고를 지어내지 마세요. 모르는 내용은 모른다고
  답하고 고객센터 문의를 안내하세요.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 3. AI 응답 저장 (is_admin=true로 저장해 기존 위젯에서 "받은 메시지"로 표시됨)
