...
import { Package, Truck, CheckCircle, LogOut, ShieldCheck, Filter, Heart, ShoppingBag, ExternalLink, MapPin, User, Settings, Save, Plus, Trash2, Home, Briefcase, Star, Search, Loader2, X, Map, ShoppingCart, ChevronRight, CreditCard, ClipboardCheck, MessageSquare, Box, Camera, AlertCircle } from 'lucide-react';
...
function MyPageContent() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: addrData } = await supabase.from('addresses').select('*').eq('user_id', currentUser.id).order('is_default', { ascending: false });
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) throw error;
      alert(language === 'ko' ? '리뷰가 소중하게 등록되었습니다!' : 'Review submitted!');
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 border-b border-border-light pb-8">
          <div>
            ...
            <p className="text-muted text-sm font-light">
              <span className="font-medium text-deep-sage border-b border-deep-sage/30 pb-0.5">{profile.full_name || user.email}</span> {language === 'ko' ? ' 님, 반갑습니다.' : ' welcome back.'}
            </p>
          </div>
                      ...
                      {order.order_items?.map((item: any, idx: number) => (
                        <div key={idx} className="flex flex-col md:flex-row md:items-center justify-between gap-8 group">
                          <div className="flex gap-6">
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="pt-2 flex gap-2">
                                {order.status === '배송완료' && (
                                  <button onClick={() => handleOpenReview(item.products)} className="text-[10px] border border-deep-sage text-deep-sage px-3 py-1.5 rounded-sm flex items-center gap-1.5 hover:bg-deep-sage hover:text-white transition-all font-medium">
                                    <MessageSquare className="w-3 h-3" /> 리뷰 쓰기
                                  </button>
                                )}
                                <button onClick={() => router.push(`/shop/${item.products?.id}`)} className="text-[10px] border border-border-light px-3 py-1.5 rounded-sm flex items-center gap-1.5 hover:bg-hanji-white transition-all text-charcoal/60"><Box className="w-3 h-3" /> 재구매</button>
                              </div>
                            </div>
                          </div>
                          <div className="flex md:flex-col gap-2">
                            {order.status === '결제완료' && (
                              <>
                                <button onClick={() => handleUpdateStatus(order.id, '배송완료')} className="text-[10px] bg-deep-sage/10 text-deep-sage px-5 py-2.5 rounded-sm flex items-center gap-2 hover:bg-deep-sage hover:text-white transition-all uppercase tracking-widest min-w-[120px] justify-center"><CheckCircle className="w-3 h-3" /> 배송완료 처리 (테스트용)</button>
                                <button className="text-[10px] border border-border-light text-muted px-5 py-2.5 rounded-sm flex items-center gap-2 hover:bg-terracotta hover:text-white hover:border-terracotta transition-all uppercase tracking-widest min-w-[120px] justify-center"><X className="w-3 h-3" /> 주문 취소</button>
                              </>
                            )}
                            {order.status === '배송완료' && (
                              <button className="text-[10px] border border-charcoal text-charcoal px-5 py-2.5 rounded-sm flex items-center gap-2 hover:bg-charcoal hover:text-white transition-all uppercase tracking-widest min-w-[120px] justify-center"><ClipboardCheck className="w-3 h-3" /> 구매 확정 완료</button>
                            )}
                          </div>
                        </div>
                      ))}
          ...
          {activeTab === 'addresses' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => setIsAddrModalOpen(true)} className="bg-charcoal text-white px-6 py-3 rounded-sm hover:bg-deep-sage transition-all text-[10px] uppercase tracking-[0.2em] flex items-center gap-2 shadow-lg"><Plus className="w-4 h-4" /> {language === 'ko' ? '새 배송지 추가' : 'Add New Address'}</button>
              </div>
                ...
                {addresses.map((addr) => (
                  <div key={addr.id} className={`p-8 bg-white border rounded-sm relative group transition-all ${addr.is_default ? 'border-deep-sage ring-1 ring-deep-sage/20' : 'border-border-light hover:border-deep-sage/50'}`}>
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center gap-2"><span className="font-serif text-lg text-charcoal">{addr.address_name}</span>{addr.is_default && <span className="bg-deep-sage text-white text-[8px] px-2 py-0.5 rounded-full uppercase tracking-tighter">{t.mypage.defaultAddress}</span>}</div>
                    ...
                    <div className="space-y-2 text-[13px] text-charcoal/80 font-light leading-relaxed"><p className="font-medium">{addr.receiver_name} / {addr.receiver_phone}</p><p>({addr.postcode}) {addr.address}</p><p>{addr.detail_address}</p></div>
                    {!addr.is_default && (<button onClick={() => handleSetDefaultAddress(addr.id)} className="mt-6 text-[10px] text-deep-sage hover:underline flex items-center gap-1.5"><Star className="w-3 h-3" /> {t.mypage.setDefault}</button>)}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="relative w-full max-w-xl bg-white rounded-sm shadow-2xl p-10 overflow-y-auto max-h-[90vh]">
              <div className="flex justify-between items-center mb-10 pb-4 border-b border-border-light">
                <h3 className="font-serif text-3xl">리뷰 작성하기</h3>
                <button onClick={() => setIsReviewModalOpen(false)} className="p-1 hover:rotate-90 transition-transform"><X className="w-7 h-7" /></button>
              </div>
              
              <div className="flex gap-6 mb-10 items-center bg-hanji-white/30 p-4 rounded-sm border border-border-light">
                <div className="relative w-16 h-20 bg-white rounded-sm overflow-hidden border border-border-light"><Image src={selectedProduct.imageUrl || ''} alt="" fill className="object-cover" /></div>
                <div>
                  <p className="text-[10px] text-muted uppercase tracking-widest mb-1">Product</p>
              ...
              <form onSubmit={handleSubmitReview} className="space-y-10">
                ...
                <div className="text-center space-y-4">
                  <p className="text-xs text-muted uppercase tracking-[0.2em]">상품은 만족스러우셨나요?</p>
                  <div className="flex justify-center gap-3">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button key={star} type="button" onClick={() => setReviewData({...reviewData, rating: star})} className={`transition-all ${reviewData.rating >= star ? 'text-deep-sage scale-110' : 'text-border-light hover:text-deep-sage/40'}`}>
                        <Star className={`w-10 h-10 ${reviewData.rating >= star ? 'fill-current' : ''}`} />
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] text-muted uppercase tracking-[0.1em] ml-1">솔직한 구매 후기를 들려주세요</label>
                  <textarea required value={reviewData.content} onChange={(e) => setReviewData({...reviewData, content: e.target.value})} placeholder="다른 구매자들에게 도움이 되는 정보를 공유해 주세요 (최소 10자)" rows={6} className="w-full bg-hanji-white/30 border border-border-light px-5 py-4 rounded-sm text-sm focus:border-deep-sage outline-none transition-all resize-none" />
                </div>

                <div className="grid grid-cols-4 gap-4">
                  <div className="aspect-square border-2 border-dashed border-border-light rounded-sm flex flex-col items-center justify-center text-muted hover:border-deep-sage hover:text-deep-sage transition-all cursor-pointer">
                    <Camera className="w-6 h-6 mb-2" /><span className="text-[9px] uppercase font-bold">Photo</span>
                  </div>
                </div>

                <div className="bg-hanji-white/50 p-4 rounded-sm flex items-start gap-3">
                  <AlertCircle className="w-4 h-4 text-deep-sage flex-shrink-0 mt-0.5" />
                  <p className="text-[10px] text-muted leading-relaxed">작성해주신 리뷰는 자연의 결 서비스 개선과 다른 고객님들의 구매 결정에 큰 도움이 됩니다. 정성스러운 리뷰에 감사드립니다.</p>
                </div>

                <button type="submit" disabled={isSaving || reviewData.content.length < 10} className="w-full bg-charcoal text-white py-5 rounded-sm hover:bg-deep-sage transition-all font-serif text-xl flex items-center justify-center gap-3 shadow-xl disabled:opacity-30 disabled:cursor-not-allowed">
                  {isSaving ? <Loader2 className="w-6 h-6 animate-spin" /> : <ClipboardCheck className="w-6 h-6" />}
                  리뷰 등록 완료하기
                </button>
              </form>
            </motion.div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-md bg-white rounded-sm shadow-2xl p-10 overflow-y-auto max-h-[90vh]">
              ...
              <form onSubmit={handleSaveAddress} className="space-y-8">
                <div className="space-y-2"><label className="text-[10px] text-muted uppercase ml-1">배송지 별칭</label><input required value={newAddr.address_name} onChange={(e) => setNewAddr({...newAddr, address_name: e.target.value})} placeholder="우리 집, 회사 등" className="w-full bg-hanji-white/30 border border-border-light px-5 py-3.5 rounded-sm text-sm focus:border-deep-sage outline-none transition-all" /></div>
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2"><label className="text-[10px] text-muted uppercase ml-1">받는 분 성함</label><input required value={newAddr.receiver_name} onChange={(e) => setNewAddr({...newAddr, receiver_name: e.target.value})} className="w-full bg-hanji-white/30 border border-border-light px-5 py-3.5 rounded-sm text-sm focus:border-deep-sage outline-none transition-all" /></div>
                  <div className="space-y-2"><label className="text-[10px] text-muted uppercase ml-1">연락처</label><input required value={newAddr.receiver_phone} onChange={(e) => setNewAddr({...newAddr, receiver_phone: e.target.value})} placeholder="010-0000-0000" className="w-full bg-hanji-white/30 border border-border-light px-5 py-3.5 rounded-sm text-sm focus:border-deep-sage outline-none transition-all" /></div>
                </div>
